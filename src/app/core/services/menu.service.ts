...
export class MenuService{
    ...
    getAllMenus(): Observable<Menu[]> {
        ...
        return this.httpClient.get<BaseResponse[]>(this.apiUrl + '/menu')
            .pipe(
                ...
                map((response) => response.map((menu) => ({ ...menu, name: menu.dishDescription + ' pancho ' }))),
                tap((response) => this.allMenusSubject.next(response))
            );
    }

    // getMenuById(id: number): Observable<Menu[]> {
    //     return this.httpClient.get<BaseResponse[]>(`${this.apiUrl}/menu/${id}`)
    //       .pipe(
    //         map((response: BaseResponse[]) => response.map((menu) => new MenusResponse(menu))),
    //         map((response: MenusResponse[]) => response.map((menu) => MenusMapper.map(menu))),
    //         map((response) => response.map((menu) => ({ ...menu, name: menu.dishDescription + ' pancho ' }))),
    ...
    //   }
      ...
      getMenuById(id: number): Observable<Menu [] > {
        return this.httpClient.get<Menu[]>(`${this.apiUrl}/menu/${id}`)
          .pipe(
            map((menu: Menu []) => ({ ...menu })),
            tap((menu: Menu []) => console.log('Menú obtenido:', menu))
          );
      }
